...
import { salvarNomeIp, lerCookie, tokenSessaoValido, NOME_COOKIE_SESSAO } from '../../shared/admin.mjs';
...
export default async (req: Request) => {
  ...
  const nomesIps = await salvarNomeIp(store, ip, nome);

  return Response.json({ ok: true, nomesIps });
};

export const config = {
  path: '/api/admin-nomear-ip',
};
